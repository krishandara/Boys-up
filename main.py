...
def decrypt_and_extract_zip(filepath: str, fernet_key: str) -> list:
    """Decrypt Fernet-encrypted ZIP and return extracted file paths."""
    ...
    decrypted_zip_path = filepath.replace(".zip", "_decrypted.zip")
    with open(decrypted_zip_path, 'wb') as f:
        f.write(decrypted)

    extract_dir = decrypted_zip_path.replace(".zip", "_extracted")
...
def decrypt_appx_file(filepath: str, fernet_key: str) -> str:
    """Decrypt Fernet-encrypted PDF or ZIP, return decrypted file path."""
    fernet = Fernet(fernet_key.encode())
    with open(filepath, 'rb') as f:
        encrypted = f.read()
    decrypted = fernet.decrypt(encrypted)
    decrypted_path = filepath.replace(".zip", "_decrypted.zip").replace(".pdf", "_decrypted.pdf")
    with open(decrypted_path, 'wb') as f:
        f.write(decrypted)
    return decrypted_path

def download_aes_key_if_needed(m3u8_url: str) -> str:
    """Check and download AES key from m3u8 if present."""
    try:
        resp = requests.get(m3u8_url)
        match = re.search(r'#EXT-X-KEY:METHOD=AES-128,URI="(.*?)"', resp.text)
        if match:
            ...
            key_path = "aes_decryption.key"
            with open(key_path, 'wb') as f:
                f.write(key_data)
            return key_path
        return None
    except Exception as e:
        print(f"[AES] Failed to check or download key: {e}")
        return None

def download_m3u8_video(m3u8_url: str, output_name: str):
    """Use ffmpeg to download and decrypt m3u8 video."""
    try:
        download_aes_key_if_needed(m3u8_url)
        cmd = f'ffmpeg -allowed_extensions ALL -i "{m3u8_url}" -c copy "{output_name}"'
        os.system(cmd)
    except Exception as e:
...
async def drm_handler(client: Client, m: Message):
    text = m.text.split("\n")
    links = [line.strip() for line in text[1:] if line.strip()]

    for url in links:
        name = url.split("/")[-1].split("?")[0].split("*")[0].replace(".pdf", "").replace(".zip", "").replace(".m3u8", "")
        cczip = f"📦 Decrypted ZIP: `{name}`"
        ccpdf = f"📄 Decrypted PDF: `{name}`"
        ccmp4 = f"🎞️ Video: `{name}`"
        ...
        if ".m3u8" in url:
            try:
                ...
                await client.send_document(chat_id=channel_id, document=output_file, caption=ccmp4)
                os.remove(output_file)
            except Exception as e:
                await m.reply_text(f"❌ Failed to download m3u8 video: {e}")
                continue

        # zip
        elif ".zip" in url:
            if '*' in url:
                enc_url, key = url.split("*")
                filename = f"{name}.zip"
                os.system(f'yt-dlp -o "{filename}" "{enc_url}"')
                try:
                    extracted_files = decrypt_and_extract_zip(filename, key)
                    for file_path in extracted_files:
                        await client.send_document(chat_id=channel_id, document=file_path, caption=os.path.basename(file_path))
                        os.remove(file_path)
                    os.remove(filename)
                    os.rmdir(os.path.dirname(extracted_files[0]))  # remove extracted dir
                except Exception as e:
                    await m.reply_text(f"❌ ZIP decryption failed: {e}")
                    continue
            else:
                os.system(f'yt-dlp -o "{name}.zip" "{url}"')
                await client.send_document(chat_id=channel_id, document=f"{name}.zip", caption=cczip)
        ...
        # pdf
            ...
            if '*' in url:
                ...
                os.system(f'yt-dlp -o "{filename}" "{enc_url}"')
                try:
                    decrypted = decrypt_appx_file(filename, key)
                    await client.send_document(chat_id=channel_id, document=decrypted, caption=ccpdf)
                    os.remove(filename)
                    os.remove(decrypted)
                except Exception as e:
                    await m.reply_text(f"❌ PDF decryption failed: {e}")
                    continue
            else:
                os.system(f'yt-dlp -o "{name}.pdf" "{url}"')
                await client.send_document(chat_id=channel_id, document=f"{name}.pdf", caption=ccpdf)
                os.remove(f"{name}.pdf")

        # fallback
        else:
            try:
                await m.reply_text("⏬ Downloading file...")
...
async def echo_handler(client: Client, m: Message):
    await m.reply_text("Use /drm followed by your links.")
...
async def start(bot, m: Message):
    ...
    if m.chat.id in AUTH_USERS:
        await start_message.edit_text(
            ...
            f"If you face any problem contact -  [](https://t.me/)\n", disable_web_page_preview=True, reply_markup=BUTTONSCONTACT
        )
    else:
